...
- **Was:** JavaScript-Client für Supabase (Auth, PostgREST, Storage) — die
  Datenschicht des Cloud-Wegs `app.bit-atelier.de` (Phase 57).
...
- **Warum hier gelistet, obwohl MIT sonst über package.json nachvollziehbar
  ...
  mit dem Browser ausgelieferte Datenschicht. In den `demo`- und `lokal`-Builds
  ist das Paket per Tree-Shaking NICHT enthalten (57-02).
